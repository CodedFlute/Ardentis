...
SX1262 radio = new Module(4, 1, 10, 6, RadioSPI);
...
void setup() {
  ...
  Serial.println("[INFO] Ground station started.");

  Serial.println("[INFO] Initializing Radio...");
  RadioSPI.begin(RADIO_SPI_SCK, RADIO_SPI_MISO, RADIO_SPI_MOSI, RADIO_NSS);

  int state = radio.begin(FREQ, BANDWIDTH, SF, CR);

  if(state == RADIOLIB_ERR_NONE) {
    Serial.println(F("[INFO] Radio Started!"));
  } else {
    Serial.print(F("[ERROR] Radio failed to start. Code: "));
...
void loop() {
  ...
  Serial.println("Ground station running...");
  ...
  if(receivedFlag) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      APRSMessage msg;
      msg.decode(str);
      Serial.print(F("[APRS] Decoded Message: "));
      Serial.println(msg.toString());


    } else if (state == RADIOLIB_ERR_CRC_MISMATCH) {
      // packet was received, but is malformed
      Serial.println(F("[SX1262] Error Receiving Data: CRC error!"));

    } else {
      // some other error occurred
      Serial.print(F("[SX1262] Error Recieving Data. Code: "));
      Serial.println(state);
